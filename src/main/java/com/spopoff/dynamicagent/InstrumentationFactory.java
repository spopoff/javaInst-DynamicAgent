 ...
 * under the License.    
 */
package com.spopoff.dynamicagent;

import com.spopoff.oursinstrument.OursTransformer;
import java.io.File;
...
public class InstrumentationFactory {
     ...
     */
    public static synchronized Instrumentation getInstrumentation(final Logger log) {
        if (log.isTraceEnabled() == true) {
            log.trace(_name + ".getInstrumentation() _inst:" + _inst
                + " _dynamicallyInstall:" + _dynamicallyInstall);
        }else{
            log.info("get Instrumentation");
        }
            ...
            public Object run() {
                ...
                try {
                    if (!InstrumentationFactory.class.getClassLoader().equals(
                        ...
                        log.warn("pas le bon loader="+InstrumentationFactory.class.getClassLoader());
                        //return null;
                    }
                } catch (Throwable t) {
                    log.error("erreur recup loader="+t.getLocalizedMessage());
                    return null;
                }
                File toolsJar = null;
                ...
                if (vendor.isIBM() == false) {
                    ...
                    if (toolsJar == null) {
                        log.error("pas trouvé tools.jar");
                        return null;
                    }
                }
                ...
                if (vmClass == null) {
                    log.error("erreur chargement instance VM");
                    return null;
                }
                String agentPath = getAgentJar(log);
                if (agentPath == null) {
                    log.error("erreur sur path agentJar null");
                    return null;
                }
                loadAgent(log, agentPath, vmClass);
                log.debug("fin de l'exec protégée");
                return null;
            }// end run()
        });
    ...
    public static void agentmain(String agentArgs, Instrumentation inst) {
        ...
        System.out.println("inside agentMain");
        OursTransformer oursTrans = new OursTransformer();
        inst.addTransformer(oursTrans, true);
    }
     ...
     */
    private static String createAgentJar(File directory) throws IOException {
//        File file =
//            File.createTempFile(InstrumentationFactory.class.getName(), ".jar");
        File file =
            File.createTempFile(InstrumentationFactory.class.getName(), ".jar", directory);
        file.deleteOnExit();
        System.out.println("fichier tempo="+file.getAbsolutePath());
        ZipOutputStream zout = new ZipOutputStream(new FileOutputStream(file));
        System.out.println("fichier tempo zip ajoute META-INF/MANIFEST.MF");
        zout.putNextEntry(new ZipEntry("META-INF/MANIFEST.MF"));

        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(zout))) {
            ...
            System.out.println("mis à jour manifest");
        }
     ...
     */
    private static File findToolsJar(Logger log) {
        ...
        if (toolsJarFile.exists() == false) {
            ...
            if (log.isTraceEnabled() == true) {
                ...
                log.debug("trouvé tools="+toolsJarFile.getAbsolutePath());
            }
            return toolsJarFile;
        }
    }
     ...
     * happens.
     */
    private static String getAgentJar(Logger log) {
        ...
        if (cs == null || agentJarFile == null
            ...
            createJar=true;
            directory = agentJarFile.getParentFile()
                    .getParentFile().getParentFile().getParentFile().getParentFile();
            directory = new File(directory+"/lib");
            log.debug("Il faut créer le jar dans ="+directory);
        }
        ...
        if (createJar == true) {
            ...
            try {
                agentJar = createAgentJar(directory);
                if (log.isInfoEnabled() == true) {
                    log.info(_loc.get("temp-file-creation", agentJar).getMessage());
                }else{
                    log.debug(_loc.get("temp-file-creation", agentJar).getMessage());
                }
                ...
                if (log.isTraceEnabled() == true) {
                    ...
                    log.error(".getAgentJar() caught unexpected="+ioe.getLocalizedMessage());
                }
                agentJar = null;
            }
        } else {
     ...
     */
    private static void loadAgent(Logger log, String agentJar, Class<?> vmClass) {
        try {
            ...
            if (log.isTraceEnabled() == true) {
                ...
                log.error("loadAgent() caught an exception="+t.getLocalizedMessage());
            }
        }
    }
     ...
     */
    private static Class<?> loadVMClass(File toolsJar, Logger log, JavaVendors vendor) {
        try {
            ...
            if (log.isTraceEnabled()) {
                ...
                log.error("loadVMClass() failed to load the VirtualMachine class");
            }
        }
        return null;
    }
     ...
     */
    private static boolean validateAgentJarManifest(File agentJarFile, Logger log,
        ...
        try {
            ...
            if (log.isTraceEnabled() == true) {
                ...
                log.error("validateAgentJarManifest() caught unexpected="+e.getLocalizedMessage());
            }
        }
        return false;
